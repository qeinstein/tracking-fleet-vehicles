...
export default function FilterControlBar({
  ...
  return (
    <div className="absolute bottom-4 sm:bottom-5 left-1/2 -translate-x-1/2 z-20 panel rounded-2xl shadow-panel-lg p-2 max-w-[calc(100vw-1.5rem)] sm:max-w-[calc(100vw-2rem)]">
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar sm:overflow-visible sm:flex-wrap sm:justify-center">
        {/* District filter */}
        ...
        <div className="relative flex items-center flex-shrink-0">
          ...
          <input
            ...
            className="w-32 sm:w-48 bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-accent font-mono transition-colors"
          />
        </div>

        <div className="w-px h-7 bg-slate-200 mx-0.5 flex-shrink-0" />

        {/* Fleet size — "show more cars" */}
        <div className="relative flex items-center flex-shrink-0" title="Number of vehicles in the fleet">
        ...
        <button
          onClick={onToggle3D}
          className={`flex-shrink-0 px-3 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 border transition-colors ${
            viewMode3D
